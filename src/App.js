...
function App() {
  ...
  const downloadToDevice = async () => {
    ...
    downloadFile(csvBlob, `${invoiceNumber}_QuickBooks.csv`);
    ...
    downloadFile(iifBlob, `${invoiceNumber}_QuickBooks.iif`);
    ...
    downloadFile(instructionsBlob, 'QuickBooks_Import_Instructions.txt');
  ...
  const shareFiles = async () => {
    ...
    try {
      ...
      const csvContent = generateQuickBooksCSV();
      const csvBlob = new Blob([csvContent], { type: 'text/csv' });
      const csvFile = new File([csvBlob], `${invoiceNumber}_QuickBooks.csv`, { type: 'text/csv' });
      files.push(csvFile);
  ...
  const generateQuickBooksInstructions = () => {
    return `QUICKBOOKS 2013 IMPORT INSTRUCTIONS
====================================

This folder contains files formatted for QuickBooks 2013:

1. ${invoiceNumber}_QB2013.csv
   - CSV format for importing invoice data
   - Use: File > Utilities > Import > Excel Files
   - Compatible with QuickBooks 2013 data import

2. ${invoiceNumber}_QB2013.iif
   - IIF format for direct invoice import
   - Use: File > Utilities > Import > IIF Files
   - Creates complete invoice transaction

IMPORT STEPS FOR IIF FILE:
1. Open QuickBooks 2013
2. Go to File > Utilities > Import > IIF Files
3. Select the ${invoiceNumber}_QB2013.iif file
4. Click Import
5. The invoice will appear in your Invoice list

IMPORT STEPS FOR CSV FILE:
1. Open QuickBooks 2013
2. Go to File > Utilities > Import > Excel Files
3. Select "Customers" or "Invoices" depending on what you want to import
4. Follow the import wizard
5. Map the columns to QuickBooks fields

NOTES:
- Customer "${customerName}" may need to be created first
- Tax settings should match your QuickBooks tax setup
- Item codes will be created as needed
- Invoice date: ${invoiceDate}
- Total amount: $${calculateTotal().toFixed(2)}

For support, refer to QuickBooks 2013 Help > Import Data.`;
  };
  ...
  // Generate QuickBooks 2013 compatible CSV
  const generateQuickBooksCSV = () => {
    const lines = [];
    
    // Enhanced CSV Header for QuickBooks 2013 Invoice Import
    lines.push('"Invoice#","Customer","Date","Item","Description","Quantity","Rate","Amount","Tax Code","Tax Amount","Customer Phone","Customer Email","Terms","Due Date","Memo"');
    ...
    items.filter(item => item.description).forEach((item) => {
      ...
      lines.push(`"${invoiceNumber}","${customerName}","${invoiceDate}","${cleanDescription}","${cleanDescription}",${item.quantity},${item.price},${lineTotal.toFixed(2)},"Tax",${taxAmount.toFixed(2)},"${customerPhone || ''}","${customerEmail || ''}","Net 30","${dueDateStr}","${memoText}"`);
    });
    
    // Add summary line for total invoice
    const grandTotal = calculateTotal();
    lines.push(`"${invoiceNumber}","${customerName}","${invoiceDate}","TOTAL","Invoice Total",1,${grandTotal.toFixed(2)},${grandTotal.toFixed(2)},"","","${customerPhone || ''}","${customerEmail || ''}","Net 30","${dueDateStr}","${documentType.toUpperCase()} Total"`);
    
    return lines.join('\n');
  };
  
  // Generate QuickBooks IIF format for direct import
  const generateQuickBooksIIF = () => {
    ...
    lines.push('HDR\tQuickBooks Pro\t2013\tRelease\tR1\t20130101\t' + new Date().toLocaleDateString() + '\t' + new Date().toLocaleTimeString() + '\tCash');
    ...
    lines.push(`TRNS\tINVOICE\t${invoiceDate}\tAccounts Receivable\t${customerName}\t\t${invoiceTotal.toFixed(2)}\t${invoiceNumber}\t${documentType.toUpperCase()} for ${customerName}\tN\tN\t\t\t\t\t\t\t\t\tN\t`);
    ...
    items.filter(item => item.description).forEach(item => {
      const lineTotal = (parseFloat(item.quantity) || 0) * (parseFloat(item.price) || 0);
      lines.push(`SPL\tINVOICE\t${invoiceDate}\tIncome\t${customerName}\t\t-${lineTotal.toFixed(2)}\t${invoiceNumber}\t${item.description}\tN\t${item.quantity}\t${item.price}\t${item.description}\tUnpaid\tY\tTax\t${invoiceDate}\t\t\t\t\t\t\t${invoiceDate}\t`);
    });
    
    // Tax line if applicable
    if (taxTotal > 0) {
      lines.push(`SPL\tINVOICE\t${invoiceDate}\tSales Tax Payable\t${customerName}\t\t-${taxTotal.toFixed(2)}\t${invoiceNumber}\tSales Tax\tN\t\t\t\tUnpaid\tN\t\t${invoiceDate}\t\t\t\t\t\t\t${invoiceDate}\t`);
    }
